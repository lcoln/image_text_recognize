...
def get_redThresh(gray_img, min_color, max_color, ratio):
  # 获取双峰值差距，通过调参来扩大二值化范围，达到尽可能收缩轮廓的效果
  gap = int(abs(max_color - min_color) / 10)
  if ratio < 0.5:  
    # 白底黑字
...
def get_font_color_ratio(originImg, min_color, max_color):
  ...
  gray_img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
  RedThresh = get_redThresh(gray_img, min_color, max_color, 1)
